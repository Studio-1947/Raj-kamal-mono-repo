...
const GenericSalesWidget: React.FC<Props> = ({
  ...
  const apiSeries = summary?.timeSeries || [];
  // If there are totals but no dates, synthesize a flat series so the chart remains visible.
  const series = useMemo(() => {
    ...
    if (totalAmt > 0 || totalOrders > 0) {
      ...
      const avg = Math.round(totalAmt / Math.max(1, days));
      return [
        { date: since.toISOString().slice(0, 10), total: avg },
        { date: now.toISOString().slice(0, 10), total: avg },
      ];
    }
    return [] as { date: string; total: number }[];
  }, [apiSeries, counts?.totalAmount, counts?.totalCount, days]);

  const isSyntheticSeries = useMemo(
    () => apiSeries.length === 0 && series.length === 2,
    [apiSeries.length, series.length]
  );
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 text-gray-900 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={series}>
              ...
              <Line
                ...
                strokeWidth={2}
                dot={false}
                strokeDasharray={isSyntheticSeries ? '4 4' : undefined}
              />
            </LineChart>
          </ResponsiveContainer>
        ) : (
      ...
      {isSyntheticSeries && !loading && (
        <div className="mt-2 text-xs text-gray-500">Averaged fallback shown due to missing per-day dates.</div>
      )}
      ...
      {!loading && isDev && (error || fromCache) && (
        <div className="mt-2 rounded-md bg-amber-50 p-2 text-xs text-amber-800">
          {fromCache ? 'Showing cached data. ' : 'Dev note: '}
