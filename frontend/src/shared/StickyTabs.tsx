...
export default function StickyTabs({ tabs, active, onChange, className }: StickyTabsProps) {
  return (
    <div
      className={
        "flex flex-nowrap gap-2 overflow-x-auto whitespace-nowrap custom-scrollbar " + (className || "")
      }
    >
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={
              "shrink-0 rounded-2xl px-5 pb-3 pt-5 text-sm font-semibold transition-all " +
              (isActive
                ? "bg-[#0067B5] text-white shadow-[0_-2px_10px_rgba(0,0,0,0.08)]"
