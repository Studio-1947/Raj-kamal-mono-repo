 ...
 *  - blogId (optional)
 */
router.get('/brand-summary/posts', authenticateToken, async (req: AuthRequest, res: Response) => {
	try {
		...
		if (!from || Number.isNaN(Date.parse(from))) {
			res.status(400).json({ success: false, error: "Invalid or missing 'from' query param. Use ISO datetime string." });
			return;
		}
		if (!to || Number.isNaN(Date.parse(to))) {
			res.status(400).json({ success: false, error: "Invalid or missing 'to' query param. Use ISO datetime string." });
			return;
		}

		const base = 'https://app.metricool.com/api/v2/analytics/brand-summary/posts';
		...
		if (!resp.ok) {
			...
			res.status(502).json({ success: false, error: 'Metricool upstream error', status: resp.status, body: text });
			return;
		}
