#!/usr/bin/env node
/**
 * Import script for Lok Event Sales Excel workbook.
 * Usage: npx tsx backend/src/scripts/import-lok-sales.ts [--file "./data/file.xlsx"] [--chunk 500]
 */
...
// Column aliases for normalization (Lok Event sheet has minimal columns)
const COLS = {
...
/** Map a raw Excel row into the LokEventSale create input */
function mapRow(raw: Record<string, any>): Prisma.LokEventSaleCreateManyInput {
  ...
  const mapped: Prisma.LokEventSaleCreateManyInput = {
    orderNo: null,
    orderStatus: null,
    month: null,
    year: null,
  ...
  (mapped as any).rowHash = sha256Hex(`lok-${key}-${qtyRaw || 0}`);
  return mapped;
}
...
async function importWorkbookFromDisk(opts?: { filePath?: string; chunkSize?: number }) {
  ...
  // Only process the "Lok Event" sheet
  const targetSheet = 'Lok Event';
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    const toInsert: Prisma.LokEventSaleCreateManyInput[] = [];
    ...
    for (let i = 0; i < toInsert.length; i += chunkSize) {
      ...
      try {
        const res = await prisma.lokEventSale.createMany({ data: chunk, skipDuplicates: true });
        inserted += res.count;
  ...
  if (errors.length > 0) {
    ...
    errorReportPath = path.join(dataDir, `import-lok-errors-${ts}.json`);
    try {
