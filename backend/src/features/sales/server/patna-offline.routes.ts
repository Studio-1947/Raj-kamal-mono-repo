...
router.get("/summary", async (req, res) => {
  ...
  try {
    ...
    const revenueByStateRows = await prisma.$queryRaw<any[]>(Prisma.sql`
      ...
      FROM "patna_offline_sales" ${whereClause}
    ...
    console.error("patna_summary_failed", e);
  ...
  try {
    ...
    const [agg] = await prisma.$queryRaw<any[]>(Prisma.sql`
      SELECT
        ...
        (SELECT TRIM("binding") FROM "patna_offline_sales" ${whereClause} AND "binding" IS NOT NULL AND TRIM("binding") != '' GROUP BY 1 ORDER BY COUNT(*) DESC LIMIT 1) AS top_binding
      FROM "patna_offline_sales"
      ${whereClause}
    `);
    ...
    console.error("patna_counts_failed", e);
...
router.get("/options", async (req, res) => {
  try {
    const [titles, customers, publishers, authors, states, cities, bindings, types] = await Promise.all([
      prisma.patnaOfflineSale.findMany({ select: { title: true }, distinct: ['title'], take: 10000 }),
      prisma.patnaOfflineSale.findMany({ select: { customerName: true }, distinct: ['customerName'], take: 10000 }),
      prisma.patnaOfflineSale.findMany({ select: { publisher: true }, distinct: ['publisher'], take: 5000 }),
      prisma.patnaOfflineSale.findMany({ select: { author: true }, distinct: ['author'], take: 5000 }),
      prisma.patnaOfflineSale.findMany({ select: { state: true }, distinct: ['state'], take: 100 }),
      prisma.patnaOfflineSale.findMany({ select: { city: true }, distinct: ['city'], take: 1000 }),
...
// GET /api/patna-offline-sales/daily-details
router.get("/daily-details", async (req, res) => {
  ...
  try {
    ...
    console.error("patna_daily_details_failed", e);
