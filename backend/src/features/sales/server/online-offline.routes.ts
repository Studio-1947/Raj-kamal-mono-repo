...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const items = await prisma.onlineOfflineSale.findMany(args);
    ...
    const totalCount = await prisma.onlineOfflineSale.count({ where });
...
// GET /api/online-offline-sales/summary
router.get("/summary", async (req, res) => {
  try {
    const [agg, topItems] = await Promise.all([
      prisma.onlineOfflineSale.aggregate({
...
// GET /api/online-offline-sales/counts
router.get("/counts", async (req, res) => {
  try {
    const agg = await prisma.onlineOfflineSale.aggregate({
      _count: { _all: true },
      _sum: { amount: true },
    });

    const uniqueCustomers = await prisma.onlineOfflineSale.groupBy({
      by: ['customerName'],
      _count: { _all: true },
    });
...
// GET /api/online-offline-sales/options
...
// GET /api/online-offline-sales/daily-details
router.get("/daily-details", async (req, res) => {
  ...
  try {
    ...
    console.error("online_daily_details_failed", e);
