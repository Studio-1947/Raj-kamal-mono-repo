...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ ok: false, error: "Failed to fetch lok event sales" });
  }
});
...
router.get("/counts", async (req, res) => {
  ...
  try {
    const items = await prisma.lokEventSale.findMany({
      select: {
        amount: true,
        qty: true,
        rate: true,
    ...
    for (const r of items) {
      ...
      // Calculate amount - LokEvent data often has amount field OR qty * rate
      let amt = decToNumber(r.amount);
      if (!amt) {
        const raw = r.rawJson as Record<string, any> | undefined;
        const v = numSafe(
    ...
    console.error("lok_event_sales_counts_failed", e);
