...
function buildUrl(
  endpoint: string,
  searchParams?: MetricoolRequestOptions["searchParams"],
): string {
...
async function fetchWithRetry(
  ...
  try {
    ...
    if (response.status === 429 && retries > 0) {
      ...
      console.warn(
        `Rate limited (429). Retrying after ${delayMs}ms. Retries left: ${retries - 1}`,
      );
      await sleep(delayMs);
    ...
    if (retries > 0 && error instanceof Error && error.name !== "AbortError") {
      ...
      console.warn(
        `Request failed. Retrying after ${delayMs}ms. Retries left: ${retries - 1}`,
      );
      await sleep(delayMs);
      return fetchWithRetry(url, options, retries - 1);
    }
  ...
  return queueRequest(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `[Metricool] Request failed: ${options.method ?? "GET"} ${url}`,
        );
        console.error(
          `[Metricool] Status: ${response.status} ${response.statusText}`,
        );
        console.error(
          `[Metricool] Response payload:`,
          JSON.stringify(payload, null, 2),
        );

        if (response.status === 429) {
          errorMessage =
            "Rate limit exceeded. The Metricool API is temporarily unavailable. Please wait a moment and try again.";
        } else {
          errorMessage = `${errorMessage} - Details: ${JSON.stringify(payload)}`;
        }
